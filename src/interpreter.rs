...
use crate::bezier::SVG;


pub fn get(path: &str) -> SVG {
    let dim = read_dim(path);
    let path_lines = read_paths(path);
    let beziers = parse_paths(path_lines);
...
pub fn read_paths(path: &str) -> Vec<String> {
...
pub fn read_dim(path: &str) -> (f64, f64) {
    let br = BufReader::new(std::fs::File::open(path).unwrap());
    let mut width = 0.0;
    let mut height = 0.0;
    for line_result in br.lines() {
        if let Ok(line) = line_result {
            if line.contains("width=") && width == 0.0 {
                width = line.split("width=\"").collect::<Vec<&str>>()[1].split("\"").collect::<Vec<&str>>()[0].parse::<f64>().unwrap();
            }
            if line.contains("height=") && height == 0.0 {
                height = line.split("height=\"").collect::<Vec<&str>>()[1].split("\"").collect::<Vec<&str>>()[0].parse::<f64>().unwrap();
            }
        }
    }
    (width, height)
...
pub fn parse_paths(mut svg: Vec<String>) -> Vec<Bezier> {
...
// THE FOLLOWING IS GENERATED BY AI (GPT4o) AND UNTESTED
