...
impl BezierType {
    pub fn new_c(p0: Point, p1: Point, p2: Point, p3: Point) -> Self {
        BezierType::Cubic([p0, p1, p2, p3])
    }

    pub fn new_q(p0: Point, p1: Point, p2: Point) -> Self {
        BezierType::Quadratic([p0, p1, p2])
    }

    pub fn new_l(p0: Point, p1: Point) -> Self {
        BezierType::Line([p0, p1])
    }

    pub fn point_at(&self, t: f64) -> Result<Point, Err> {
        if t > 1.0 || t < 0.0 {
            return Err("t must be between 0 and 1");
        }
        match self {
            BezierType::Cubic(points) => {
